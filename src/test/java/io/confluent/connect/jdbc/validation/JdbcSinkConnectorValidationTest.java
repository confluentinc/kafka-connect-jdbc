...
public class JdbcSinkConnectorValidationTest {
  ...
  public void validate_withDeleteEnabledErrorAndConnectionError_setsMultipleErrors() throws Exception {
    ...
    // Should have errors for both delete.enabled, pk.mode, and connection.url
    // However, connection validation is skipped when there are config errors
    assertErrors(2);
    assertErrors(DELETE_ENABLED, 1);
    assertErrors(PK_MODE, 1);
    EasyMock.verify(mockDialect);
  ...
  @Test
  public void validate_withEmptyConnectionUrl_setsConfigDefError() {
    ...
    assertTrue(results.configValues()
        .stream()
        .anyMatch(cv -> cv.name().equals(CONNECTION_URL) && !cv.errorMessages().isEmpty()));
  }
