...
public class DateTimeUtilsTest {
  ...
  /**
   * The calendar-aware overloads render a pre-1582 value differently per calendar system: the same
   * milliseconds are 1500-01-01 in the hybrid Julian/Gregorian calendar and 1500-01-10 in the
   * proleptic one. Input is built with a UTC {@link GregorianCalendar} so the result does not depend
   * on the JVM default zone.
   */
  @Test
  public void testFormatDateHonorsCalendarSystem() {
    java.util.Date hybrid1500 = new java.util.Date(utcHybridMillis(1500, Calendar.JANUARY, 1));

    assertEquals("1500-01-01",
        DateTimeUtils.formatDate(hybrid1500, utcZoneId, DateCalendarSystem.LEGACY));
    assertEquals("1500-01-10",
        DateTimeUtils.formatDate(hybrid1500, utcZoneId, DateCalendarSystem.PROLEPTIC_GREGORIAN));
  }

  @Test
  public void testFormatTimestampHonorsCalendarSystem() {
    java.util.Date hybrid1500 = new java.util.Date(utcHybridMillis(1500, Calendar.JANUARY, 1));

    assertEquals("1500-01-01 00:00:00.000",
        DateTimeUtils.formatTimestamp(hybrid1500, utcZoneId, DateCalendarSystem.LEGACY));
    assertEquals("1500-01-10 00:00:00.000",
        DateTimeUtils.formatTimestamp(hybrid1500, utcZoneId,
            DateCalendarSystem.PROLEPTIC_GREGORIAN));
  }

  /**
   * Post-1582 the calendars agree, so both settings render identically and match the two-argument
   * overload. That is what makes the default safe for ordinary data.
   */
  @Test
  public void testCalendarSystemIsIrrelevantAfterTheGregorianCutover() {
    java.util.Date epoch = new java.util.Date(0L);

    assertEquals("1970-01-01",
        DateTimeUtils.formatDate(epoch, utcZoneId, DateCalendarSystem.LEGACY));
    assertEquals("1970-01-01",
        DateTimeUtils.formatDate(epoch, utcZoneId, DateCalendarSystem.PROLEPTIC_GREGORIAN));
    assertEquals(DateTimeUtils.formatDate(epoch, utcZoneId),
        DateTimeUtils.formatDate(epoch, utcZoneId, DateCalendarSystem.LEGACY));
    assertEquals(DateTimeUtils.formatTimestamp(epoch, utcZoneId),
        DateTimeUtils.formatTimestamp(epoch, utcZoneId, DateCalendarSystem.LEGACY));
  }

  /**
   * The overloads render in the supplied zone, not the JVM default, so a connector honours
   * {@code db.timezone} regardless of where it runs.
   */
  @Test
  public void testFormatHonorsSuppliedZoneNotJvmDefault() {
    TimeZone.setDefault(TimeZone.getTimeZone("Asia/Kolkata"));
    java.util.Date epoch = new java.util.Date(0L);

    for (DateCalendarSystem system : DateCalendarSystem.values()) {
      assertEquals("1970-01-01 00:00:00.000",
          DateTimeUtils.formatTimestamp(epoch, ZoneOffset.UTC, system));
      assertEquals("1969-12-31 19:00:00.000",
          DateTimeUtils.formatTimestamp(epoch, ZoneId.of("America/New_York"), system));
    }
  }

  /**
   * A {@code java.sql.Date} must be accepted: its {@code toInstant()} throws by contract, and the
   * proleptic conversion produces one, so the overloads must not route through it.
   */
  @Test
  public void testFormatAcceptsJavaSqlSubclasses() {
    long millis = utcHybridMillis(1970, Calendar.JANUARY, 1);

    assertEquals("1970-01-01", DateTimeUtils.formatDate(
        new java.sql.Date(millis), utcZoneId, DateCalendarSystem.PROLEPTIC_GREGORIAN));
    assertEquals("1970-01-01 00:00:00.000", DateTimeUtils.formatTimestamp(
        new java.sql.Timestamp(millis), utcZoneId, DateCalendarSystem.PROLEPTIC_GREGORIAN));
  }
