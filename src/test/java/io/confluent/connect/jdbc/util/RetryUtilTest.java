...
public class RetryUtilTest {
  ...
  public void shouldRetryOnTransientException() {
    ...
    assertTrue("Transient exceptions should be retried", RetryUtil.shouldRetry(exception));
  }
  ...
  public void shouldNotRetryOnNonTransientException() {
    ...
    assertFalse("Non transient exceptions must not be retried", RetryUtil.shouldRetry(exception));
  }

  @Test
  public void shouldRetryWhenErrorCodeRegistered() {
    int customErrorCode = 987654;
    RetryUtil.addRetriableErrorCodes(Collections.singleton(customErrorCode));
    SQLException exception = new SQLException("custom error", "S0001", customErrorCode);

    assertTrue("Registered error codes should be retried", RetryUtil.shouldRetry(exception));
  }
}
