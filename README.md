...
위의 동적 날짜 생성을 이용해 로테이션된 테이블을 쿼리했을 때 해당 테이블이 로테이션 지연 등의 이유로 아직 존재하지 않으면 커넥터에서 에러가 발생한다. 이런 경우 로테이션된 테이블외에 현재 테이블에 들어온 내용도 Kafka 로 가져오기 못하기에 최신 데이터 수급에 지연이 생긴다. 
