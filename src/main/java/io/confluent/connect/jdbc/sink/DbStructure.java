...
public class DbStructure {
  ...
  void create(
    ...
    if (!config.autoCreate) {
      throw new ConnectException(String.format(
          "Table %s is missing and auto-creation is disabled",
          tableName
      ));
    }
  ...
  boolean amendIfNecessary(
    ...
    final Set<SinkRecordField>
        missingFields =
        missingFields(fieldsMetadata.allFields.values(), dbColumns.keySet());
    ...
    for (SinkRecordField missingField : missingFields) {
      if (!missingField.isOptional() && missingField.defaultValue() == null) {
        throw new ConnectException(
            "Cannot ALTER to add missing field "
            + missingField
